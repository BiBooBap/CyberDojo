...
class CourseService {
  static async getAllCourses(username) {
    ...
    return await Promise.all(courses.map(async course => {
      ...
      if(username) {
        isEnrolled = enrolledCourses.some(enrolledCourse => enrolledCourse.course_id === course._id);
        testExists = await TestService.getTestExistsForUserAndCourse(username, course._id);
      }
